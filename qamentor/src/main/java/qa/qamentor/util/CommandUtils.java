...
import qa.qamentor.CommonMethods.CamSoda;
...
public class CommandUtils {
	...
	public Object executeAction(WebDriver driver, WebElement element,
		...
		case "ValidateToken":		
			if(inputData == null && referenceStep != null && !referenceStep.trim().equals(""))
			{
				String[] input=referenceStep.split(",");
				int input1 = new Integer(input[0]);
				int input2 = new Integer(input[1]);
				String value1=getTextMap.get(Integer.valueOf(input1));
				String value2=getTextMap.get(Integer.valueOf(input2));
				returnObj = CamSoda.addValue(value1, value2);	
				getTextMap.put(stepNo, returnObj.toString());
			}					   
			break;	
		case "CamSodaLogin":
			returnObj = CamSoda.login(driver);
			getTextMap.put(stepNo, returnObj.toString());
			break;
		case "CamSodaLogout":
			CamSoda.logOut(driver);
			break;
		case "ClickHDbutton":
			CamSoda.click(driver, element);
			break;
		case "ClickGoPrivateButton":
			returnObj = CamSoda.ClickPrivateButton(driver);
			break;
		case "EndPrivateChat":
			CamSoda.ClickEndPrivateChat(driver);
			break;
		case "EndSpyChat":
			CamSoda.ClickEndSpyChat(driver);
			break;
		case "ClickPrivateButton":
			returnObj = CamSoda.PrivateButton(driver);
			break;
		case "ClickSpyShowButton":
			returnObj = CamSoda.SpyShowButton(driver);
			break;
